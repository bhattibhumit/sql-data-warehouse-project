...
WITH yearly_product_sales AS (
    SELECT 
        YEAR(f.order_date) AS Yearly_performance,
...
SELECT
    ...
    LAG(Sales) OVER (PARTITION BY product_name ORDER BY Yearly_performance) AS py_sales,
    Sales - LAG(Sales) OVER (PARTITION BY product_name ORDER BY Yearly_performance) AS diff_py,
    CASE 
        WHEN Sales - LAG(Sales) OVER (PARTITION BY product_name ORDER BY Yearly_performance) > 0 THEN 'Increase'
        WHEN Sales - LAG(Sales) OVER (PARTITION BY product_name ORDER BY Yearly_performance) < 0 THEN 'Decrease'
        ELSE 'No Change'
    END AS py_change
FROM yearly_product_sales
ORDER BY product_name, Yearly_performance;
